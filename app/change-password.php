...
if(isset($_POST['newPassSubmit'])) //the user has entered a new pass- lets update it
{
    ...
    {
                header("Location: change-password2.php?error=missingfields&email=".$email."&token=".$token);
    ...
    {
      header("Location: change-password2.php?error=invalidpass&email=".$email."&token=".$token);
    ...
    {
              header("Location: change-password2.php?error=passnotmatch&email=".$email."&token=".$token);
